...
interface TeamApprovalProps {
  onApprove: (reviewData: { memberId: string; comment: string; signature?: string }) => void;
...
export function TeamApproval({ onApprove, currentUserId }: TeamApprovalProps) {
  ...
  const teamMembers: TeamMember[] = (formData.basicInfo.teammates || []).map((teammate, index) => ({
    id: index.toString(),
    name: teammate.name,
    role: 'Team Member',
  ...
  const currentUserApproved = teamMembers.find((m) => m.id === currentUserId)?.approved;
